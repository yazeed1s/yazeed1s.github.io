...
Traditional servers bundle CPU, memory, and storage into one box, so if you need more RAM you buy a bigger box or add DIMMs if you haven't hit the motherboard limit, and if you don't use all your RAM it just sits idle and you can't share it.
...
The OS problems are hard though, and the paper mostly focuses on what's still unsolved, so memory allocation at scale, scheduling with memory locality, pointer sharing across servers, failure handling for "optional" memory, and security for hot-swappable pools, all of which need fundamental rethinking.

The timeline matches what happened with storage disaggregation, where you start small with a few hosts per pool, add switches for rack-scale, then push the fabric boundary outward. Whether it ends up being "CXL over something" or something else is open, but the trajectory rhymes with how storage disaggregation went.

## where it fits and where it doesn't

It's good for data-intensive workloads like Spark or Ray or distributed DBs that spend cycles serializing and copying, working sets that barely fit in local memory, clusters with memory imbalance, and places where memory is already 50%+ of server cost. It's bad for workloads that already fit in local memory since you're just adding latency for no reason, latency-sensitive apps that can't handle hundreds of extra nanoseconds, and traditional apps that don't share data across processes anyway. Pool memory is slower than local, hundreds of ns versus ~100ns, but still way faster than SSD or disk, so for workloads that currently page to disk this could be big, but for workloads that don't page at all adding a slower tier might just make things worse.
