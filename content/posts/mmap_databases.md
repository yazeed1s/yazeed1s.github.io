...
For a database this looks perfect at first, since you map data files, access pages through pointers, let the kernel decide what stays in memory, and skip writing a buffer pool. Several databases tried it, and most backed away from it.
...
With mmap you need workarounds, so MongoDB's MMAPv1 engine used `MAP_PRIVATE` to create a copy-on-write workspace, which ends up with two copies of the database in memory, and SQLite copies pages to user-space buffers before modifying them, which defeats the purpose of mmap, and LMDB uses shadow paging, which forces single-writer concurrency. All of these are complex, and all of them give back the simplicity that mmap was supposed to provide.
...
With mmap a range scan hits a page fault on every cold page, one after another, and each one blocks. You can try `madvise(MADV_SEQUENTIAL)` but it's a hint, not a guarantee, and it doesn't help for non-sequential access patterns.
...
With a buffer pool error handling is centralized, so you read a page, check the checksum, and handle I/O errors all in one place.
...
The collapse happens because of TLB shootdowns, since when the kernel evicts a page it has to invalidate the TLB entry on every CPU core that might have it cached, and CPUs don't keep TLB entries coherent automatically, so the kernel sends inter-processor interrupts that cost thousands of cycles each, and under heavy eviction the TLB shootdowns hit 2 million per second.
...
If your entire dataset fits in memory and you're read-only, mmap works, since eviction never kicks in so you avoid TLB shootdowns, and transactional write hazards don't apply. LMDB operates in this sweet spot for some workloads.
...
For me this comes down to one thing, which is that the OS page cache is general-purpose while databases need very specific control, and general-purpose is fine for generic workloads but databases have specific access patterns, durability rules, and error-handling paths that the OS cannot infer.

It's similar to the tiered memory problem where the OS tries to manage page placement transparently, but transparency breaks down when the application knows something the kernel doesn't. Buffer pool versus mmap is the same tension, do you trust the OS abstraction, or do you manage things yourself because you know your workload better?
